...
#.\windows_pipeline.ps1 -server labelremittance -form_id covid_endline_household -start_timestamp 0 -username falamos@poverty-action.org -password "Vivienda:(1PA" -columns_with_attachments "audio_audit_survey,comments,text_audit,sstrm_conversation,sstrm_sound_level" -outputs_path "X:\Box\CP_Projects\IPA_PHL_Projects\Labeled Remittances\COVID study\Data\endline_all" -server_key "X:\Box\CP_Projects\IPA_PHL_Projects\Labeled Remittances\COVID study\Questionnaire & programming\Programming\key\covid_endline_PRIVATEDONOTSHARE.pem"
...
if($server_key -eq $null){ 
   ...
   echo $server_key
   curl.exe -u "${username}:${password}" -F 'private_key=@"'"${server_key}"'"' -o ${json_file_path} ${url}
   #curl.exe -u "${username}:${password}" -F 'private_key=@"X:\Box\CP_Projects\IPA_PHL_Projects\Labeled Remittances\COVID study\Questionnaire & programming\Programming\key\covid_endline_PRIVATEDONOTSHARE.pem"' -o ${json_file_path} ${url}
}
